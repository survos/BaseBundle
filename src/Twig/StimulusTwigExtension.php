...
final class StimulusTwigExtension extends AbstractExtension
{
    public function getFunctions(): array
    {
        return [
            ...
            new TwigFunction('survos_stimulus_target', [$this, 'renderStimulusTarget'], ['needs_environment' => true, 'is_safe' => ['html_attr']]),
        ];
    }
    ...
    public function renderStimulusController(Environment $env, $dataOrControllerName, array $controllerValues = []): string
    {
        ...
        foreach ($data as $controllerName => $controllerValues) {
            ...
            $values = $this->renderData($env, $controllerValues, $controllerName, '-value');
     ...
     * @throws \Twig\Error\RuntimeError
     */
    public function renderStimulusAction(Environment $env, $dataOrControllerName, string $actionName = null, string $eventName = null, array $actionParams = []): string
    {
        if (\is_string($dataOrControllerName)) {
        ...
        $values = $this->renderData($env, $actionParams, $controllerName, '-params');

        return 'data-action="'.implode(' ', $actions).'" '.implode(' ', $values);
    ...
    public function renderStimulusTarget(Environment $env, $dataOrControllerName, string $targetNames = null, array $params = []): string
    {
        ...
        }, array_keys($targets), $targets)).implode(' ', $targetData);
    }
